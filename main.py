...
Prevents crashes if Colab silently switches to CPU.

Uses correct pipeline task names for Transformers v5+.

Ensures Whisper runs on GPU when available.

🚀 Next Step
Once this block works, the rest of your clipping pipeline should run fine. If you still see GPU not being used, check Colab’s runtime type and confirm CUDA is installed (!nvidia-smi).

Would you like me to rewrite your full script with these fixes integrated (including the corrected summarizer output key and GPU detection everywhere), so you can just copy‑paste into Colab and run without debugging?

Today
write full code
Friday, Jun 5
Here’s the full corrected Colab‑ready script with GPU detection, updated summarizer pipeline, and audio‑safe video download. You can copy‑paste this into a Colab cell and run it directly:
